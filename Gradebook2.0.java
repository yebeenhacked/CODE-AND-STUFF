...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while(true){
            ...
            if(optionSelectIndex == 0){
                ...
                addSection(gradebook.Main.get(gradebook.csec));
                return;
            }
            else{
    ...
    static void addStudent(student stu){
        try{
            writer = new FileWriter("Data.txt" , true);
            writer.write("\n");
            writer.write("Next Student:\n");
            writer.write(stu.name + "\n");
            writer.write(stu.lastname+ "\n");
            writer.write(stu.username+ "\n");
            writer.write((int) stu.phone+ "\n");
            writer.write(stu.tards+ "\n");
            writer.write(stu.abs+ "\n");
            writer.write(stu.name + "'s stuff:\n");
            writer.write("\n");
            writer.close();

            for(String a:stu.assiments.keySet()){
                as aa = stu.assiments.get(a);
                addAs(aa);
            }
    ...
    static void addSection(sec m){
        try{
            writer = new FileWriter("Data.txt" , true);
            writer.write(m.name + "\n");
            writer.write(m.name + "'s Students");
            writer.write("\n");
            writer.close();
            for(String a: m.students.keySet()) {
                student aa = m.students.get(a);
                addStudent(aa);
